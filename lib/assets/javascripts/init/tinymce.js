...
$(document).ready(function() {
  ...
  var file_selection = function(id){
    return function(sel){
      if(sel){//catch the url, the little_path is inside model repository.rb
        $('#'+id).val(sel[0].little_path.replace(/\?\d+$/, ''));
      ...
      console.log($('#'+id).val(sel[0].little_path.replace(/\?\d+$/, '')));
    }
  };
  ...
  var editorSetup = function(ed){
    ed.on('NodeChange', function(e){
      if (e && e.element.nodeName.toLowerCase() == 'img'){
        ...
        //This function set width and height as null. In beta test - Patryck
        ...
        if(match_url){
          ...
          var image_big = match_url[1] + "b" + match_url[3];

          tinyMCE.DOM.setAttribs(e.element, {
            // Set srcset in the img tags
            'srcset': image_med + " 800w, " + image_big + " 1200w"
          });
        }
  ...
  $('textarea.mceAdvance').tinymce({
    ...
    extended_valid_elements : "div[*],span[*],iframe[srcset|src|'width|height|name|align|frameborder|scrolling|id|onload],applet[code|codebase|archive|name|id|width|height|param],video[*],source[*],audio[*]",
